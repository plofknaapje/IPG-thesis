...
def inverse_LSG(obss: list[lsg.LSG], sols: list[np.ndarray]) -> tuple:
    ...
    while True:
        ...
        # Support variables
        true_f = model.addVars(len(observations), len(incumbents), len(customers),
                                lb=0, ub=1, name="true_f")
        alt_f_indices = [(o, i, j, s) for o in observations for i in incumbents
                         for j in customers for s in range(len(sub_sols[o, i]))]
        alt_f = model.addVars(alt_f_indices, lb=0, ub=1, name="alt_f")

        true_profit = model.addVars(len(observations), len(incumbents), name="true_profit")
        alt_profit_indices = [(o, i, s) for o in observations for i in incumbents 
        ...
        for o, obs in enumerate(obss):
            ...
            for i in incumbents:
                ...
                model.addConstrs(
                    true_f[o, i, j] *
                    gp.quicksum(gp.quicksum(sols[o][ii, k] * util[o, ii, j, k]
                                            for ii in incumbents if ii != i) +
                                sols[o][i, k] * util[o, i, j, k] for k in viable[j]) <=
                ...
                for s, alt_sol in enumerate(sub_sols[o, i]):
                    model.addConstrs(
                        ...
                        gp.quicksum(gp.quicksum(sols[o][ii, k] * util[o, ii, j, k]
                                                for ii in incumbents if ii != i) +
                                    alt_sol[k] * util[o, i, j, k] for k in viable[j]) <=
        ...
        for o in observations:
            for i in incumbents:
                model.addConstr(true_profit[o, i] <= 
                    gp.quicksum(obs.margin[i, j] * obs.pop_count[j] * true_f[o, i, j]
                                for j in customers) - 
                    gp.quicksum(obs.costs[i, k] * true_sol[i, k] for k in locations)
                )
                
                for s, alt_sol in enumerate(sub_sols[o, i]):
                    model.addConstr(alt_profit[o, i, s] <= 
                        gp.quicksum(obs.margin[i, j] * obs.pop_count[j] * alt_f[o, i, j, s]
                                for j in customers) - 
                        gp.quicksum(obs.costs[i, k] * alt_sol[k] for k in locations)            
                    )
                    
                    model.addConstr(delta[o, i] >= alt_profit[o, i, s] - true_profit[o, i])
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        error = (alpha - learned_alpha) ** 2 + (betas[0] - learned_betas[0])**2 + (betas[1] - learned_betas[1])**2
        errors.append(error)
