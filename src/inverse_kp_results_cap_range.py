...
if approach is Target.WEIGHTS:
    for cap in caps:
        ...
        for i in range(repeats):
            ...
            problems = generate_weight_problems(
                size=observations[-1], n=n, r=r, capacity=cap, corr=True, rng=rng
            )
        ...
        for j, o in enumerate(observations):
            results.append([cap, o, np.mean(runtimes[j]), np.mean(error[j])])
            print(results[-1])

    df = pd.DataFrame(results, columns=columns)
    df.to_csv(
        f"./results/kp/inverse_kp-weights-{repeats}-{n}-items-{r}.csv",
        float_format="%6.3f",
        index=False,
    )
    results = []
    ...
    df = pd.DataFrame(results, columns=columns)
    df.to_csv(
        f"./results/kp/inverse_kp-payoffs-{repeats}-{n}-items-{r}.csv",
        float_format="%6.3f",
        index=False,
    )
    results = []
