...
def local_inverse_weights(problem: KnapsackPackingGame, verbose=False) -> np.ndarray:
    ...
    while True:
        ...
        for p in problem.players:
            ...
            if (
                problem.obj_value(p, greedy_solution, new_player_x)
                >= problem.obj_value(p, greedy_solution) + eps
            ):
