...
def generate_weight_problems(
    ...
    while len(problems) < size:
        ...
        norm_interactions = interactions / np.maximum(
            np.abs(interactions).sum(axis=2)[:, :, np.newaxis], 1
        )
        for i in range(n):
            norm_interactions[i, i, :] = 0

        problem = KPG(
            norm_weights, norm_payoffs, norm_interactions / inter_factor, capacity
        )
        ...
        for i, problem in enumerate(problems):
            for j in players:
                ...
                if tuple(new_solution) in solutions[i, j]:
                    continue
                new_value = new_solution @ p[j] + gp.quicksum(
...
if __name__ == "__main__":
    ...
    match approach:
        case "weight":
            ...
            inverse, eps = inverse_eps_opt(
                inverse_weights, weight_problems, values, pows
            )
