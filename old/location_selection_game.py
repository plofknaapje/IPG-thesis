...
class LocationSelectionGame:
    ...
    def update_alpha_beta(self, alpha: float, betas: list):
        self.alpha = alpha
        self.betas = betas
        self.utility = np.array(
            [
                [
                    [
                        betas[i]
                        + alpha * self.distance[j, k]
                        + (1 - alpha) * self.conveniences[k]
...
def random_LSG(
    ...
    for jj, pop in enumerate(pop_coords):
        for kk, store in enumerate(store_coords):
            # Euclidian distance
            distance[jj, kk] = (
                (pop[0] - store[0]) ** 2 + (pop[1] - store[1]) ** 2
            ) ** 0.5
            norm_distance[jj, kk] = (max_dist - distance[jj, kk]) / max_dist
    viable = [[kk for kk in k if distance[jj, kk] < max_dist] for jj in j]

    # Convenience of locations
    competition_coords = [
        (random.randint(1, 100), random.randint(1, 100)) for _ in range(50)
    ]
    conveniences = []
    for kk, store in enumerate(store_coords):
        comp_dists = []
    ...
    utility = np.array(
        [
            [
                [
                    betas[ii]
                    + alpha * norm_distance[jj, kk]
                    + (1 - alpha) * conveniences[kk]
                    for kk in k
                ]
                for jj in j
            ]
...
def solve_LSG(problem: LocationSelectionGame, verbose=False) -> np.ndarray:
...
def solve_partial_LSG(
    problem: LocationSelectionGame, x_hat: np.ndarray, i: int, verbose=False
) -> np.ndarray:
    ...
    bound_m = [
        min(problem.utility[i, j, k] for k in problem.locations) / 1
    ...
    for location in problem.locations:
        if location not in problem.potential_locs[i]:
            x[location].ub = 0
    y = model.addMVar(
        (len(problem.locations), len(problem.locations)), lb=0, ub=1, name="y"
    )
    ...
    model.setObjective(
        gp.quicksum(
            problem.margin[i, j] * problem.pop_count[j] * f[j]
            for j in problem.customers
        )
        - gp.quicksum(problem.costs[i, k] * x[k] for k in problem.locations),
        GRB.MAXIMIZE,
    )
    ...
    y_u = [
        gp.quicksum(y[j, k] * problem.utility[i, j, k] for k in viable[j])
        for j in problem.customers
    ]
    x_hat_u = [
        gp.quicksum(
            f[j] * x_hat[ii, k] * problem.utility[ii, j, k]
            for ii in rivals
            for k in viable[j]
        )
        for j in problem.customers
